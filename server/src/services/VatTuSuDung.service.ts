...
} from "../contracts/VatTuSuDung.contract";
...
export class VatTuSuDungService extends BaseService {
  ...
  async getContracts(limit: number = 10, page: number = 1) {
    try {
      ...
      if (danhSachVatTuSuDung && danhSachVatTuSuDung.length > 0) {
        ...
        const totalPage =
          limit != 0 ? Math.ceil(danhSachVatTuSuDung.length / limit) : 1;

        const startIndex = (page - 1) * limit;
        const endIndex =
          startIndex + limit > danhSachVatTuSuDung.length
            ? danhSachVatTuSuDung.length
            : startIndex + limit;
        const danhSachVatTuSuDungChoosed =
          startIndex == endIndex
            ? danhSachVatTuSuDung
            : danhSachVatTuSuDung.slice(startIndex, endIndex);
  ...
  async getContractById(id_VatTuSuDung: number) {
    try {
      const vatTuSuDung = await this._VatTuSuDungContract.getContractById(
        id_VatTuSuDung
      );
    ...
    try {
      ...
      const danhSachVatTuSuDungFilterById_HoatDongNhatKy =
        danhSachVatTuSuDung?.filter(
          (element) =>
            element.returnValues.id_HoatDongNhatKy == id_HoatDongNhatKy
        );

      if (
        danhSachVatTuSuDungFilterById_HoatDongNhatKy &&
        danhSachVatTuSuDungFilterById_HoatDongNhatKy.length > 0
      ) {
        const danhSachVatTuSuDungFilterById_HoatDongNhatKyFilter = [];
        const totalPage =
          limit != 0
            ? Math.ceil(
                danhSachVatTuSuDungFilterById_HoatDongNhatKy.length / limit
              )
            : 1;
        ...
        const endIndex =
          startIndex + limit >
          danhSachVatTuSuDungFilterById_HoatDongNhatKy.length
            ? danhSachVatTuSuDungFilterById_HoatDongNhatKy.length
            : startIndex + limit;
        const danhSachVatTuSuDungFilterById_HoatDongNhatKyChoosed =
          startIndex == endIndex
            ? danhSachVatTuSuDungFilterById_HoatDongNhatKy
            : danhSachVatTuSuDungFilterById_HoatDongNhatKy.slice(
                startIndex,
                endIndex
              );
        ...
        for (let nhatKyDongRuong of danhSachVatTuSuDungFilterById_HoatDongNhatKyChoosed) {
          ...
          danhSachVatTuSuDungFilterById_HoatDongNhatKyFilter.push(
            nhatKyDongRuongTemp
          );
        }
        ...
        const result = {
          ...
          page: page,
          danhSachVatTuSuDung:
            danhSachVatTuSuDungFilterById_HoatDongNhatKyFilter,
        };
