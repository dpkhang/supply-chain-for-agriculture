import { LoHangSanPham, LoHangSanPhamContract } from "./../contracts/LoHangSanPham.contract";
import { GiaoDichMuaBanSanPham, GiaoDichMuaBanSanPhamContract } from "./../contracts/GiaoDichMuaBanSanPham.contract";
...
export class GiaoDichMuaBanSanPham_Service {
  ...
  getContracts = async (limit = 10, page = 1) => {
    ...
    console.log(giaoDichMuaBanSanPham)
    const results = [];
    if (giaoDichMuaBanSanPham && giaoDichMuaBanSanPham.length > 0) {
      ...
      for (const contract of giaoDichMuaBanSanPhamLimit) {
				const loHangSanPham = await this._loHangSanPhamContract.getContractById(contract.returnValues.id_LoHang)
